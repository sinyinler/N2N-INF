...
### 2026-06-11 — 纠错 + 转入自主模式 + 干净的多帧方案
- **纠错(重要)**：我之前在 BS-INF 里搭的单帧/融合 L2R **都是失败的弱复现**（轻量0.07M backbone、缺 per-image 归一化/坏点修补、训练太短），不能代表 L2R 真实水平。用户的 L2R 项目 exp1(DRUNet 2M+完整预处理) 去噪很干净。**因此"单帧能跑/融合崩=多帧泄漏"的结论作废**——我拿两个没烤熟的复现对比，不可信。
- **用户出门，转自主模式**：持续实验+记录，**结果必须看仔细**(放大细血管/对照noisy+reference/看残差有无血管结构/和能跑的基线比)，绝不"不行说行"。
- **吃透 L2R 项目**：预处理=坏点修补+稳健裁剪+log1p+**per-image归一化[0,1]**(泛化关键)；DRUNet(2M)；TTA去噪；代理指标(corr_res_den泄漏↓/vessel_ret细血管/new_edge造假↓/bg_std/HF_down)。
- **新方案(避开泄漏)**：放弃"喂邻帧给去噪器"(易泄漏)，改 **"去相关多帧对齐+平均 → 单帧 L2R"**。静止下对齐=直接平均；L2R始终单输入、结构上不可能泄漏；平均=BFI物理上估计血流信号的正道。完全复用 L2R 跑通的 train.py/DRUNet/重腐蚀/指标，唯一变量=输入单帧 vs M帧平均。
- **实验(在 L2R 项目做)**：单帧臂(L2R训mix单帧) vs 多帧臂(L2R训M帧平均图)，大平均(100+帧)当伪干净reference，比 vessel_ret/泄漏/造假+细血管放大。源数据 lightweight_G/mix(1000帧)。

### 2026-06-11 — 自主探索完整结论（在 L2R 项目内做的多帧实验）
全程严格判读(放大细血管+对照ref+看残差+量化指标), 不靠单一指标。证据图在 L2R/runs/{compare,motion,varyM,rotmotion}/。
1. **静止**: 多帧(9平均)+L2R corr_to_ref 0.97 > 单帧L2R 0.88 > 纯平均0.95; vessel_ret 0.59 vs 单帧0.16。多帧能救单帧磨掉的细血管。
2. **运动(平移±8px)**: naive平均糊(0.85) → 相位相关对齐救回(0.95, 残余0.13px)。临床手抖须先对齐。
3. **诚实纠偏**: aligned+L2R 的 corr_to_ref 最高但**肉眼过度平滑细血管**(corr被大血管主导=假象)。L2R磨血管。
4. **★vary-M(关键)**: 对齐+平均更多帧, bg_std↓ **且** hf_corr细血管↑ 单调同改善(M25: 0.985/0.78≈ref), **纯赚不赔零幻觉**; 而学习去噪是"背景↓但细血管↓"的trade-off。**帧充足时 对齐+平均 完胜 SINF/N2N/L2R。**
5. **旋转运动**: 仅平移对齐不够(hf 0.30), 须 ECC欧氏对齐(hf 0.70)。配准要匹配运动类型。
- **★最终建议**: BFI 多帧充足场景 → **去相关取帧 + ECC对齐 + 平均 M≈15-25**(满足全部4目标+运动鲁棒, 免训练)。交付工具 `L2R/bfi_mf_denoise.py`(已在真实数据验证)。学习去噪仅在帧稀缺时才值得(会磨血管)。
- **开放**: 非刚性/形变运动需光流配准(未测); 帧稀缺时 averaging+轻量L2R 的折中(未做)。
